...
const COURSE_CODES = [
  'IT1010', 'IT1020', 'IT2010', 'IT2020', 'IT3020', 'IT3030', 'SE3040', 'SE3050', 'CS4010', 'CS4020', 'BM2010', 'EN3010'
];
...
const CreateGroup = () => {
  ...
  const validateForm = () => {
    ...
    if (!form.moduleSubject.trim()) return 'Module/Subject is required.';
    if (!form.description.trim()) return 'Group Description is required.';
    if (form.memberLimit < 2) return 'Member limit must be at least 2.';
    return '';
  };
    ...
    >
      <div className="mx-auto w-full max-w-5xl space-y-6">
        <header className="rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-xl p-6 md:p-8">
          <Link to="/groups" className="inline-flex items-center gap-2 text-sm text-slate-300 hover:text-white transition-colors">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <section className="rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-xl p-6 md:p-8 space-y-6">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-xl bg-blue-500/20 text-blue-300 flex items-center justify-center"><FiTarget /></div>
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="h-11 w-full rounded-xl border border-white/10 bg-white/5 px-3 text-slate-100 placeholder:text-slate-500 outline-none focus:border-blue-400/70"
                />
                <div className="app-surface-soft rounded-xl max-h-40 overflow-y-auto">
              ...
              <label className="space-y-2">
                ...
                >
                  {FACULTIES.map((faculty) => (
                    <option key={faculty} value={faculty} className="bg-[var(--app-surface-soft)] text-slate-100">{faculty}</option>
                  ))}
                </select>
              </label>
              ...
              <div className="space-y-2">
                ...
                {avatarPreview ? (
                  <img src={avatarPreview} alt="Group avatar preview" className="h-16 w-16 rounded-xl border border-white/20 object-cover" />
                ) : null}
              </div>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-xl p-6 md:p-8 space-y-6">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-xl bg-amber-500/20 text-amber-300 flex items-center justify-center"><FiUsers /></div>
              <div>
          ...
          <section className="rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-xl p-6 md:p-8 space-y-6">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-xl bg-emerald-500/20 text-emerald-300 flex items-center justify-center"><FiCheckCircle /></div>
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <FeatureToggle icon={<FiBookOpen className="text-blue-300" />} title="Shared Materials" description="Enable a dedicated folder for PDFs and notes." checked={form.sharedMaterials} name="sharedMaterials" onChange={handleFieldChange} />
              <FeatureToggle icon={<FiMessageSquare className="text-amber-300" />} title="Discussion Forum" description="Enable a Q&A message board for members." checked={form.discussionForum} name="discussionForum" onChange={handleFieldChange} />
              <FeatureToggle icon={<FiFileText className="text-emerald-300" />} title="Task Tracker" description="Enable a simple to-do list for milestones." checked={form.taskTracker} name="taskTracker" onChange={handleFieldChange} />
            </div>
          </section>
          ...
          <div className="flex flex-wrap items-center gap-3 pb-10">
            <button
              ...
              className="app-btn-primary h-12 px-6 rounded-xl font-semibold border border-amber-400/50 hover:brightness-110 transition disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {isSubmitting ? 'Creating Group...' : 'Create Group'}
            </button>
            <Link to="/groups" className="h-12 px-5 rounded-xl font-semibold text-slate-200 border border-white/15 bg-white/5 hover:bg-white/10 transition inline-flex items-center">
              Cancel
            </Link>
          </div>
        </form>
...
const FeatureToggle = ({ icon, title, description, checked, name, onChange }) => (
  <label className="rounded-xl border border-white/10 bg-white/5 p-4 flex flex-col gap-4 cursor-pointer hover:bg-white/10 transition-colors">
    ...
    <input type="checkbox" name={name} checked={checked} onChange={onChange} className="h-5 w-5 accent-amber-400" />
  </label>
);
