...
import { 
  ...
  FiPaperclip, FiAlertCircle, FiPlus, FiClock, FiSend, FiSearch, FiTrash2 
} from 'react-icons/fi';
...
const Group = () => {
  ...
  // MEMBER 4 DYNAMIC STATES
  const [tasks, setTasks] = useState([]); // Duty 4
  const [messages, setMessages] = useState([]); // Duty 3
  const [reminders, setReminders] = useState([
    { id: 1, text: "Welcome to the squad!", type: "info" }
  ]); // Duty 10 & 7
  ...
  if (groupId) {
    if (isLoading) {
      return (
        <div className="min-h-screen bg-[#0a0d17] text-slate-200 flex items-center justify-center">
    ...
    if (!activeGroup) {
      return (
        <div className="min-h-screen bg-[#0a0d17] text-slate-200 flex flex-col items-center justify-center px-6 text-center">
          <p className="text-slate-300 font-bold mb-2">Group not found</p>
          <p className="text-slate-500 text-sm mb-6">The group may have been deleted or you don’t have access.</p>
          ...
          <div className="max-w-[1800px] mx-auto flex justify-between items-center">
            ...
            <div className="hidden md:block w-64">
              ...
              <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
                <div className="h-full bg-blue-500 w-[65%] transition-all duration-700 shadow-[0_0_10px_#3b82f6]"></div>
              </div>
            </div>
          </div>
        </header>
                ...
                <form onSubmit={handleAddTask} className="bg-white/5 p-6 rounded-3xl border border-white/10 flex flex-wrap gap-4 items-end backdrop-blur-sm">
                  <div className="flex-1 min-w-[200px]">
                    ...
                    <input 
                      ...
                      className="w-full bg-[#0d111c] border border-white/10 rounded-xl px-4 py-2.5 outline-none focus:border-blue-500 text-sm" 
                  ...
                  <div>
                    ...
                    <select 
                       ...
                       className="bg-[#0d111c] border border-white/10 rounded-xl px-4 py-2.5 outline-none text-sm cursor-pointer"
                    >
          ...
          <aside className="lg:col-span-4 p-8 space-y-8 bg-white/[0.01]">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                {reminders.map(rem => (
                  <div key={rem.id} className="bg-amber-500/5 border border-amber-500/20 p-4 rounded-2xl animate-in zoom-in-95 duration-300">
  ...
  return (
     <div className="min-h-screen bg-[#0a0d17] text-slate-200 relative pt-32 px-6">
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between mb-12">
            ...
            <Link to="/groups/create" className="bg-amber-500 text-[#0a0d17] px-8 py-4 rounded-2xl font-black hover:bg-amber-400 transition-all shadow-xl shadow-amber-500/20">
              + New Group
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {isLoading ? (
              <div className="col-span-full py-20 flex justify-center"><div className="w-10 h-10 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div></div>
